...
protoFile = "openpose_tutorial/pose/coco/pose_deploy_linevec.prototxt"
weightsFile = "openpose_tutorial/pose/coco/pose_iter_440000.caffemodel"
...
while True:
    ...
    # image1 = cv2.imread(f"data/frame{frame}.jpg")
    if frame == 0:
        plt.imshow(image1)
    ...
    print(f"Saving frame {frame} locations.")
    sio.savemat(f"detected_locations/frame{frame}.mat", {str(index): list(x[0]) for index, x in enumerate(detected_keypoints) if len(x) != 0})
    cv2.imwrite(f"detected_locations/frame{frame}_detected.jpg", frameClone)